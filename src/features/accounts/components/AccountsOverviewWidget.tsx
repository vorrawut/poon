...
export function AccountsOverviewWidget({ 
  ...
  if (error) {
    return (
      <div className={`bg-red-50 border border-red-200 rounded-xl p-6 ${className}`}>
        <div className="text-red-600 font-medium mb-2">Failed to load accounts</div>
  ...
  const accountTypeConfig: Record<AccountType, { icon: any; label: string; color: string }> = {
    checking: { icon: BanknotesIcon, label: 'Checking', color: 'from-green-500 to-emerald-600' },
    savings: { icon: BuildingLibraryIcon, label: 'Savings', color: 'from-blue-500 to-indigo-600' },
    investment: { icon: ChartBarIcon, label: 'Investments', color: 'from-purple-500 to-violet-600' },
    credit: { icon: CreditCardIcon, label: 'Credit Cards', color: 'from-red-500 to-rose-600' },
    loan: { icon: CreditCardIcon, label: 'Loans', color: 'from-orange-500 to-amber-600' },
    mortgage: { icon: BuildingLibraryIcon, label: 'Mortgage', color: 'from-gray-500 to-slate-600' },
  };

  const accountTypeCards = Object.entries(overview.accountsByType).map(([type, accounts]) => {
  ...
  return (
    <FadeIn direction="up" delay={0.1} className={className}>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">
              Last synced: {overview.lastSyncAt?.toLocaleTimeString() || 'Never'}
            </p>
          </div>
        ...
        >
          {accountTypeCards.map(({ type, config, totalBalance, accountCount, accounts }) => (
            <div 
              key={type}
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div className={`h-12 w-12 bg-gradient-to-br ${config.color} rounded-xl flex items-center justify-center text-white shadow-lg group-hover:scale-105 transition-transform`}>
              ...
              <div className="mb-2">
                <AnimatedNumber
                  ...
                  delay={0.9 + accountTypeCards.indexOf({ type, config, totalBalance, accountCount, accounts }) * 0.1}
                />
              </div>
              ...
              <div className="space-y-1">
                {accounts.slice(0, 2).map((account) => (
                  <div key={account.id} className="flex justify-between text-xs text-gray-600">
