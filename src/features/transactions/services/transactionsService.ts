import { type Transaction, type TransactionSummary, type TransactionType, type TransactionCategory } from '../types';
...
class TransactionsService {
  ...
  async fetchTransactions(limit: number = 50): Promise<Transaction[]> {
    ...
    if (shouldUseMockData()) {
      await new Promise(resolve => setTimeout(resolve, config.mockApiDelay * 0.4)); // Fastest for transactions
      console.info(`💳 Using mock transactions data (${config.environment} mode)`);
    ...
    try {
      ...
      console.warn('⚠️ API failed, falling back to mock transactions data:', error);
      ...
      await new Promise(resolve => setTimeout(resolve, config.mockApiDelay * 0.4));
      return this.getMockTransactions().slice(0, limit);
    }
  }
  ...
  async fetchTransactionSummary(): Promise<TransactionSummary> {
    ...
    const categoryCount: Record<TransactionCategory, number> = {} as any;
  ...
  private transformTransactionResponse(data: any): Transaction {
    return {
      id: data.id,
  ...
  private getMockTransactions(): Transaction[] {
    ...
    return [
      ...
      },
      {
        ...
        posted_at: new Date(now.getTime() - 1 * 24 * 60 * 60 * 1000 - 2 * 60 * 60 * 1000), // 1 day 2 hours ago
        pending: false,
        location: '',
      ...
      },
      {
        ...
        posted_at: new Date(now.getTime() - 2 * 24 * 60 * 60 * 1000 - 5 * 60 * 60 * 1000), // 2 days 5 hours ago
        pending: false,
        location: '',
      ...
      },
      {
        ...
        posted_at: new Date(now.getTime() - 3 * 24 * 60 * 60 * 1000 - 3 * 60 * 60 * 1000), // 3 days 3 hours ago
        pending: false,
        location: 'Austin, TX',
      ...
      },
      {
        ...
        posted_at: new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000 - 4 * 60 * 60 * 1000), // 7 days 4 hours ago
        pending: false,
        location: 'Austin, TX',
