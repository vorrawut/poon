...
export const THAI_CULTURAL_CATEGORIES: ThaiCulturalCategory[] = [
  {
    ...
    icon: '🙏',
    color: '#F59E0B',
    culturalContext: 'Central to Thai Buddhist culture - making merit for good karma and spiritual well-being',
    keywords: {
      en: ['temple', 'monk', 'donation', 'merit', 'buddhist', 'ceremony', 'offering', 'wat'],
  ...
  },
  {
    ...
    icon: '👨‍👩‍👧‍👦',
    color: '#EC4899',
    culturalContext: 'Filial piety (กตัญญูกตเวทิตา) is fundamental in Thai culture',
    keywords: {
      en: ['family', 'parents', 'support', 'siblings', 'relatives', 'filial', 'care'],
      th: ['ครอบครัว', 'พ่อแม่', 'ช่วยเหลือ', 'พี่น้อง', 'ญาติ', 'กตัญญู', 'ดูแล']
    },
    subcategories: [
      {
        id: 'parents_support',
  ...
  },
  {
    ...
    icon: '🎉',
    color: '#8B5CF6',
    culturalContext: 'Thai festivals involve specific traditional expenses and gift-giving customs',
    keywords: {
      en: ['songkran', 'loy krathong', 'festival', 'celebration', 'new year', 'tradition'],
  ...
  },
  {
    ...
    icon: '🤝',
    color: '#06B6D4',
    culturalContext: 'Social reciprocity (การตอบแทนบุญคุณ) is essential in Thai society',
    keywords: {
      en: ['wedding', 'funeral', 'community', 'social', 'gift', 'contribution', 'ceremony'],
      th: ['งานแต่ง', 'งานศพ', 'ชุมชน', 'สังคม', 'ของขวัญ', 'ช่วยเหลือ', 'พิธี']
    },
    subcategories: [
      {
        id: 'wedding_gifts',
  ...
  },
  {
    ...
    icon: '🌦️',
    color: '#10B981',
    culturalContext: 'Thailand\'s three distinct seasons affect spending patterns significantly',
    keywords: {
...
export function getThaiCulturalCategory(id: string): ThaiCulturalCategory | undefined {
  return THAI_CULTURAL_CATEGORIES.find(category => category.id === id);
}

// Helper function to get categories by type
export function getThaiCategoriesByKeyword(keyword: string, language: 'en' | 'th' = 'en'): ThaiCulturalCategory[] {
...
export function getLocalizedCategoryName(category: ThaiCulturalCategory, language: 'en' | 'th' = 'en'): string {
