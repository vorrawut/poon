...
export const calculateFamilyObligationBudget = (
  ...
  const recommended =
    monthlyIncome * Math.min(adjustedPercentage, maxPercentage);
  const minimum = monthlyIncome * 0.1;
  const maximum = monthlyIncome * 0.3;

  const breakdown = {
    parents: recommended * 0.6, // 60% for parents
...
export const calculateMeritMakingBudget = (
  ...
  const breakdown = {
    temple_monthly: totalBudget * 0.4, // 40% for regular temple donations
    monk_alms: totalBudget * 0.2, // 20% for daily/weekly alms
...
// Import cultural categories from centralized mock data
import { thaiCulturalCategories } from "../../../../mockData/features/thai-culture";

// Re-export for backward compatibility
export { thaiCulturalCategories };
