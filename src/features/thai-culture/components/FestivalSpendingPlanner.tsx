...
export interface FestivalExpense {
  id: string;
  category: "travel" | "gifts" | "food" | "ceremonies" | "clothing" | "entertainment" | "decorations";
  name: {
    en: string;
    th: string;
  };
    ...
    expenses: [
      {
        ...
        tips: {
          ...
          th: ["จองล่วงหน้าเพื่อราคาดีกว่า", "เปรียบเทียบราคารถบัสกับเครื่องบิน"]
        }
      },
      {
        id: "songkran-gifts",
    ...
    expenses: [
      {
        ...
        tips: {
          ...
          th: ["ใช้วัสดุที่เป็นมิตรกับสิ่งแวดล้อม", "ทำร่วมกับครอบครัวเพื่อความสัมพันธ์"]
        }
      },
      {
    ...
    >
      ...
      >
        <div className="flex items-start justify-between mb-3">
          ...
          {isUpcoming && (
            <div className="text-right">
              ...
              <div className="text-lg font-bold text-purple-300">{daysUntil}</div>
        ...
        <div className="text-xs text-gray-400">
          Range: ฿{festival.totalEstimate.min.toLocaleString()} - ฿{festival.totalEstimate.max.toLocaleString()}
        </div>
        ...
        {plannedBudget > 0 && (
          <div className="mt-2">
            <div className={cn(
              "text-sm font-medium",
              plannedBudget >= festival.totalEstimate.min && plannedBudget <= festival.totalEstimate.max
                ? "text-green-400" : plannedBudget < festival.totalEstimate.min
                ? "text-yellow-400" : "text-purple-400"
            )}>
              {plannedBudget < festival.totalEstimate.min ? "⚠️ Below minimum" :
               plannedBudget > festival.totalEstimate.max ? "💰 Above maximum" :
               "✅ Within range"}
            </div>
          </div>
        )}
...
export function FestivalSpendingPlanner({
  ...
  const [budgetPlans, setBudgetPlans] = useState<Record<string, Record<string, number>>>({});
  ...
  const totalPlannedBudget = useMemo(() => {
    return Object.values(budgetPlans).reduce((total, festivalPlan) => {
      return total + Object.values(festivalPlan).reduce((sum, amount) => sum + amount, 0);
    }, 0);
  }, [budgetPlans]);
  ...
  const handleExpenseUpdate = (festivalId: string, expenseId: string, amount: number) => {
    setBudgetPlans(prev => ({
      ...prev,
      [festivalId]: {
        ...prev[festivalId],
              ...
              >
                ...
                <div className="space-y-4">
                  {selectedFestivalData.expenses.map((expense) => {
                    const currentAmount = budgetPlans[festival.id]?.[expense.id] || 0;
                    
                    return (
                      <div key={expense.id} className="p-3 bg-gray-700/50 rounded border">
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <h5 className="font-medium">
                              {language === 'th' ? expense.name.th : expense.name.en}
                            </h5>
                            <p className="text-sm text-gray-400">
                              {language === 'th' ? expense.description.th : expense.description.en}
                            </p>
                            <span className={cn(
                              "px-2 py-1 rounded-full text-xs font-medium mt-1 inline-block",
                              expense.priority === 'essential' ? 'bg-red-500/20 text-red-300' :
                              expense.priority === 'important' ? 'bg-yellow-500/20 text-yellow-300' :
                          ...
                          <div className="text-right">
                            <div className="text-sm text-gray-400">
                              ฿{expense.estimatedCost.min.toLocaleString()} - ฿{expense.estimatedCost.max.toLocaleString()}
                            </div>
                            <div className="text-xs text-gray-500">
                              Avg: ฿{expense.estimatedCost.average.toLocaleString()}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex gap-2 flex-wrap">
                          <ThemeAwareButton
                            variant="ghost"
                            size="sm"
                            onClick={() => handleExpenseUpdate(festival.id, expense.id, expense.estimatedCost.min)}
                          >
                          ...
                          <ThemeAwareButton
                            variant="primary"
                            size="sm"
                            onClick={() => handleExpenseUpdate(festival.id, expense.id, expense.estimatedCost.average)}
                          >
                            Avg ฿{expense.estimatedCost.average.toLocaleString()}
                          </ThemeAwareButton>
                          <ThemeAwareButton
                            variant="ghost"
                            size="sm"
                            onClick={() => handleExpenseUpdate(festival.id, expense.id, expense.estimatedCost.max)}
                          >
                          ...
                          {currentAmount > 0 && (
                            <ThemeAwareButton
                              variant="ghost"
                              size="sm"
                              onClick={() => handleExpenseUpdate(festival.id, expense.id, 0)}
                ...
                <div className="mt-6 p-4 bg-amber-500/10 rounded border border-amber-500/20">
                  <h5 className="font-medium text-amber-300 mb-2">🏮 Cultural Tips</h5>
                  <ul className="text-sm text-gray-300 space-y-1">
                    {(language === 'th' ? selectedFestivalData.culturalTips.th : selectedFestivalData.culturalTips.en).map((tip, idx) => (
