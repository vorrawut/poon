...
export const identifyRiskFactors = (
  financialData: {
    monthlyIncome: number;
    monthlyExpenses: number;
    emergencyFund: number;
  ...
  // Emergency fund risk
  const monthsOfEmergency = financialData.emergencyFund / financialData.monthlyExpenses;
  ...
  const debtToIncome = (financialData.totalDebt / (financialData.monthlyIncome * 12)) * 100;
  if (debtToIncome > 30) {
    risks.push({
      id: 'high-debt',
  ...
  if (savingsRate < 10) {
    risks.push({
      ...
      recommendation: 'Review your budget and identify areas to reduce expenses or increase income.',
    });
  }
