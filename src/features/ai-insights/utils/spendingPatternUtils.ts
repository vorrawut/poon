...
export const getPatternInsights = (pattern: SpendingPattern): string[] => {
  ...
  if (pattern.confidence > 85) {
    insights.push(
      "This pattern has been consistently detected in your spending behavior.",
    );
  }

  if (pattern.impact === "high") {
    insights.push(
      "This pattern significantly impacts your overall financial health.",
    );
  }

  if (pattern.type === "recurring") {
    insights.push(
      "Consider setting up automatic savings to account for this recurring expense.",
    );
  }

  if (pattern.type === "seasonal") {
    insights.push(
      "Plan ahead for this seasonal spending pattern to avoid budget surprises.",
    );
  }

  if (pattern.type === "anomaly") {
    insights.push(
      "This unusual spending pattern may indicate a need to review your budget.",
    );
  }
