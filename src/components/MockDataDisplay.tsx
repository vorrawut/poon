...
export function MockDataDisplay() {
  ...
  return (
    <div className="bg-orange-100 border border-orange-400 rounded-lg p-4 mb-4">
      <h3 className="font-bold text-orange-800 mb-2">📊 Mock Data Direct Access</h3>
      
      {mockData.success ? (
        <div className="space-y-2 text-sm">
          <div className="bg-white p-2 rounded">
            <strong>Net Worth:</strong> ${mockData.netWorth?.totalNetWorth?.toLocaleString()}
            <br />
            <strong>Change:</strong> ${mockData.netWorth?.netWorthChange?.toLocaleString()} 
          ...
          <details className="bg-white p-2 rounded">
            <summary className="cursor-pointer font-medium">Raw Mock Data</summary>
            <pre className="text-xs mt-2 overflow-auto max-h-40">
              {JSON.stringify({ netWorth: mockData.netWorth, accounts: mockData.accounts?.slice(0, 2) }, null, 2)}
            </pre>
          </details>
        </div>
      ) : (
