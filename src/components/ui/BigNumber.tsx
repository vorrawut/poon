  ...
  return (
    <div className={`text-center ${className}`}>
      ...
      <div className="flex items-center justify-center mb-2">
        ...
        {showTooltip && (
          <InfoTooltip content={getPlainEnglishExplanation()} />
        )}
      </div>
      ...
      <p className="text-base text-gray-600 mb-4 max-w-md mx-auto">
        {format === 'currency' && value > 0 ? (
          ...
          <>You owe <strong>{formatValue(Math.abs(value))}</strong> {label.toLowerCase()}</>
        ) : (
      ...
      {changeInfo && (
        <div className={`flex items-center justify-center space-x-2 ${
          ...
          <div className={`flex items-center text-lg font-semibold ${
            ...
            <span className="mr-1">
              {changeInfo.isPositive ? '↗️' : '↘️'}
            </span>
            <span>
              {changeInfo.isPositive ? '+' : '-'}{changeInfo.formattedChange}
            </span>
          </div>
