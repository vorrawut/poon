...
export function ServiceTester() {
  ...
  if (loading) {
    return <div className="bg-blue-100 p-4 rounded">🧪 Testing services...</div>;
  }
  ...
  return (
    <div className="bg-green-100 p-4 rounded space-y-2">
      ...
      <div>
        <strong>NetWorth:</strong> {netWorthData ? `$${netWorthData.totalNetWorth?.toLocaleString()}` : 'None'}
      </div>
      
      <div>
        <strong>Accounts:</strong> {accountsData ? `${accountsData.length} accounts` : 'None'}
      </div>
      ...
      {accountsData && accountsData.length > 0 && (
        <details>
          ...
          <pre className="text-xs">{JSON.stringify(accountsData[0], null, 2)}</pre>
        </details>
      )}
    </div>
  );
