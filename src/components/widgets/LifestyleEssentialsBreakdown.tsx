...
import { Home, Sparkles, TrendingUp, AlertTriangle, Target, Lightbulb } from "lucide-react";
...
export function LifestyleEssentialsBreakdown({
  ...
  const [selectedType, setSelectedType] = useState<"essential" | "lifestyle" | "both">("both");
  ...
  const categorizedSpending = useMemo(() => {
    const categories = spendingData.reduce((acc, item) => {
    ...
    return Object.values(categories).sort((a: any, b: any) => b.total - a.total);
  }, [spendingData]);

  // Health assessment
  const getHealthStatus = () => {
    if (totals.essentialPercent > 70) return { status: "critical", message: "Essential spending is too high" };
    if (totals.lifestylePercent > 30) return { status: "warning", message: "Lifestyle spending exceeds recommended 30%" };
    if (totals.totalPercent > 90) return { status: "warning", message: "Total spending is very high" };
  ...
  const getRecommendations = () => {
    ...
    if (totals.essentialPercent > 60) {
      recommendations.push({
        ...
        message: "Essential spending is high. Look for ways to reduce utilities, subscriptions, or negotiate better rates",
  ...
  return (
    <div className={`${className}`}>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-2xl font-bold text-white mb-2">Lifestyle vs Essentials</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        <div className="bg-gradient-to-br from-indigo-900/20 to-purple-900/20 rounded-2xl p-6 border border-white/10">
          <h4 className="text-lg font-bold text-white mb-4 text-center">Spending Balance</h4>
          ...
          <div className="space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-white/80">
                  ฿{totals.essential.toLocaleString()} ({totals.essentialPercent.toFixed(1)}%)
                </span>
              </div>
              ...
              <div className="relative h-8 bg-white/10 rounded-full overflow-hidden">
                <motion.div
                  ...
                  initial={{ width: 0 }}
                  animate={{ width: `${Math.min(totals.essentialPercent, 100)}%` }}
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-white/80">
                  ฿{totals.lifestyle.toLocaleString()} ({totals.lifestylePercent.toFixed(1)}%)
                </span>
              </div>
              ...
              <div className="relative h-8 bg-white/10 rounded-full overflow-hidden">
                <motion.div
                  ...
                  initial={{ width: 0 }}
                  animate={{ width: `${Math.min(totals.lifestylePercent, 100)}%` }}
            ...
            <div className="pt-4 border-t border-white/20">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-white">
                  ฿{totals.total.toLocaleString()} ({totals.totalPercent.toFixed(1)}%)
                </span>
              </div>
          ...
          >
            <div className="flex items-center gap-2">
              {healthStatus.status === "healthy" && <Target className="w-5 h-5 text-green-400" />}
              {healthStatus.status === "warning" && <AlertTriangle className="w-5 h-5 text-yellow-400" />}
              {healthStatus.status === "critical" && <AlertTriangle className="w-5 h-5 text-red-400" />}
              
              <span className={`font-medium ${
                healthStatus.status === "healthy" ? "text-green-400" : 
                healthStatus.status === "warning" ? "text-yellow-400" : "text-red-400"
              }`}>
                {healthStatus.status.toUpperCase()}
              </span>
            </div>
            <div className="text-white/80 text-sm mt-1">{healthStatus.message}</div>
          </motion.div>
        </div>
        ...
        <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
          ...
          <div className="space-y-3 max-h-80 overflow-y-auto">
            {categorizedSpending.map((category: any, index) => {
              const showCategory = selectedType === "both" || 
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-white/80">฿{category.total.toLocaleString()}</span>
                  </div>
                  ...
                  <div className="h-2 bg-white/10 rounded-full overflow-hidden flex">
                    {category.essential > 0 && (
                      <div
                        ...
                        style={{ width: `${(category.essential / category.total) * 100}%` }}
                      />
                    )}
                    {category.lifestyle > 0 && (
                      <div
                        className="bg-pink-500 h-full"
                        style={{ width: `${(category.lifestyle / category.total) * 100}%` }}
                      />
                    )}
                  </div>
                </motion.div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {recommendations.map((rec, index) => (
                ...
                >
                  ...
                  <div className="text-white/80 text-sm mb-3">{rec.message}</div>
