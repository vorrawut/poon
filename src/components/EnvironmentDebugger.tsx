...
export function EnvironmentDebugger() {
  ...
  return (
    <div className="fixed top-4 right-4 bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-2 rounded shadow-lg text-xs z-50">
      ...
      <div><strong>Data:</strong> {shouldUseMockData() ? 'mock' : 'api'}</div>
      <div><strong>Local:</strong> {config.isLocal ? 'yes' : 'no'}</div>
      <div><strong>VITE_APP_ENV:</strong> {import.meta.env.VITE_APP_ENV || 'undefined'}</div>
    </div>
  );
}
