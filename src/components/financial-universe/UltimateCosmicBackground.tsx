  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        positions[i3 + 1] += Math.sin(time * 0.5 + i * 0.1) * 0.01 * activityMultiplier;
        positions[i3] += Math.cos(time * 0.3 + i * 0.05) * 0.005 * activityMultiplier;
        positions[i3 + 2] += Math.sin(time * 0.4 + i * 0.08) * 0.008 * activityMultiplier;
      }
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      if (meshRef.current.material instanceof THREE.MeshBasicMaterial) {
        meshRef.current.material.opacity = (0.1 + intensity * 0.2) * (0.8 + Math.sin(time * 0.3) * 0.2);
      }
    }
  });
...
function ShootingStarsManager({ 
  ...
  useEffect(() => {
    ...
    }, 2000 / (1 + financialActivity)); // More frequent with higher activity
...
function CosmicScene({ 
  ...
  return (
    <>
      {/* Ambient lighting */}
      <ambientLight intensity={0.1} color={themeMode === 'dark' ? '#1e1b4b' : '#f8fafc'} />
      ...
      {showNebula && (
        <>
          <NebulaEffect intensity={intensity} financialActivity={financialActivity} />
          <NebulaEffect intensity={intensity * 0.6} financialActivity={financialActivity * 0.7} />
        </>
      )}
