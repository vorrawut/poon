...
import type { Transaction, FilterState, SpendingTrend, CategorySpending } from '../types';
...
interface TransactionState {
  ...
  importTransactions: (accountId: string, transactions: Partial<Transaction>[]) => Promise<void>;
}
...
export const useTransactionStore = create<TransactionState>((set, get) => ({
  ...
  applyFilters: () => {
    ...
    let filtered = transactions.filter(transaction => {
      ...
      // Account filter
      if (filters.accounts.length > 0 && !filters.accounts.includes(transaction.account_id)) {
        return false;
      }
  ...
  calculateSpendingTrends: () => {
    ...
    const monthlyData: { [key: string]: { income: number; expenses: number; transactions: Transaction[] } } = {};
    ...
    const trends = Object.entries(monthlyData).map(([period, data]) => {
      ...
      const categories = Object.entries(categoryData).map(([category, info]) => ({
        category,
        amount: info.amount,
        count: info.count,
        percent: totalSpending > 0 ? (info.amount / totalSpending) * 100 : 0,
      }));
  ...
  calculateCategoryBreakdown: () => {
    ...
    const categoryData: { [key: string]: { amount: number; count: number; subcategories: { [key: string]: { amount: number; count: number } } } } = {};
    let totalSpending = 0;
    ...
    filteredTransactions
      ...
      .forEach(transaction => {
        ...
        if (!categoryData[category].subcategories[subcategory]) {
          categoryData[category].subcategories[subcategory] = { amount: 0, count: 0 };
        }
        
        categoryData[category].amount += transaction.amount;
        categoryData[category].count += 1;
        categoryData[category].subcategories[subcategory].amount += transaction.amount;
    ...
    const breakdown = Object.entries(categoryData).map(([category, data]) => ({
      ...
      subcategories: Object.entries(data.subcategories).map(([name, subData]) => ({
        name,
  ...
  importTransactions: async (accountId, transactions) => {
    ...
    try {
      ...
      set({ error: error instanceof Error ? error.message : 'Failed to import transactions' });
      throw error;
    } finally {
      set({ isLoading: false });
    }
